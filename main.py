...
# Import our modules
from enhanced_detector import EnhancedBuildingDetector
from solar_panel_optimizer import SolarPanelOptimizer
...
def check_dependencies():
    ...
    if missing:
        ...
        print("\n📦 Install them with:")
        print("pip install opencv-python numpy matplotlib scikit-learn scikit-image scipy pillow")
        return False
...
def create_sample_image():
    ...
    for (x, y, w, h) in buildings:
        ...
        # Add some texture
        for i in range(5):
            cv2.line(img, (x+i*w//5, y), (x+i*w//5, y+h), (200, 190, 180), 1)
...
def find_satellite_image():
    ...
    common_names = [
        'satellite.jpg', 'satellite.png',
        'satellite_image.jpg', 'satellite_image.png',
        'aerial.jpg', 'aerial.png',
...
def run_complete_analysis(image_path, location_config=None):
    ...
    if location_config is None:
        ...
        print(f"📍 Location: {location_config['latitude']}, {location_config['longitude']}")
    ...
    for i, result in enumerate(solar_results[:3], 1):
        ...
        print(f"      Annual energy: {result['estimated_annual_energy_kwh']:.0f} kWh/year")
    ...
    for i, cnt in enumerate(regularized):
        ...
        if M['m00'] != 0:
            cx = int(M['m10'] / M['m00'])
            cy = int(M['m01'] / M['m00'])
            cv2.putText(output_image, f"#{result['building_id']}", 
    ...
    detector.export_to_geojson(regularized, metrics, output_path='output/buildings.geojson')
    ...
    if response.lower() == 'y':
        ...
        optimizer.visualize_solar_analysis(image, regularized[building_id], top_building)
